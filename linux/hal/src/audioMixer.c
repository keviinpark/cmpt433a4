...
#define DATA_MIN (-32767)

#define LATENCY 50000
#define SAMPLE_RATE 44100
#define NUM_CHANNELS 1
#define WAVE_PCM_DATA_OFFSET 44
...
void AudioMixer_init(void)
{
    ...
    err = snd_pcm_set_params(handle,
                             ...
                             LATENCY); // 0.05 seconds per buffer
    if (err < 0) {
        printf("Playback open error: %s\n", snd_strerror(err));
        exit(EXIT_FAILURE);
    }
...
void AudioMixer_readWaveFileIntoMemory(char *fileName, wavedata_t *pSound)
{
    ...
    const int PCM_DATA_OFFSET = WAVE_PCM_DATA_OFFSET;
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
    ...
    if (!isAvailable) {
        printf("WARNING: audioMixer failed to find free soundBite slot for new wave file.\n");
    }
}
...
void AudioMixer_cleanup(void)
{
    ...
    int cancelErr = pthread_cancel(playbackThreadId);
    if (cancelErr) {
        perror("AudioMixer: failed to cancel main thread:");
        exit(EXIT_FAILURE);
    }
...
static void fillPlaybackBuffer(short *buff, int size)
{
    ...
    for (int i = 0; i < MAX_SOUND_BITES; i++) {
        ...
        for (int j = 0; j < size; j++) {
            ...
            int buffData = (int)buff[j];
            int newData = (int)(pSound->pData[location]);
            int result = buffData + newData;
            if (result > DATA_MAX) {
                result = DATA_MAX;
            } else if (result < DATA_MIN) {
                result = DATA_MIN;
            }
            buff[j] = (short)result;

            // Record that this portion of the sound bite has been played back
            // by incrementing the location inside the data where play-back
            // currently is.
            location++;
        }
...
// fetch timing statistics of filling playback buffer
void AudioMixer_getBufferStats(AudioMixer_stats_t* stats)
{
    assert(stats != NULL);

    Period_statistics_t tmp;
    Period_getStatisticsAndClear(PERIOD_EVENT_PLAYBACK_BUFFER, &tmp);
