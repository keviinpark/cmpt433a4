...
function handleCommand(socket) {
	...
	socket.on('beat', function(modeNumber) {
		console.log("Got beat command: " + modeNumber);
		relayToLocalPort(socket, "beat " + modeNumber, "beat-reply");
	});
	socket.on('vol', function(volumeNumber) {
		console.log("Got vol command: " + volumeNumber);
		relayToLocalPort(socket, "vol " + volumeNumber, "vol-reply");
	});
	socket.on('bpm', function(tempoNumber) {
		console.log("Got bpm command: " + tempoNumber);
		relayToLocalPort(socket, "bpm " + tempoNumber, "bpm-reply");
	});
	...
	socket.on('quit', function(notUsed) {
		console.log("Got quit command: ");
		relayToLocalPort(socket, "quit", "quit-reply");
	});
};
...
function relayToLocalPort(socket, data, replyCommandName) {
	...
	client.on('message', function (message, remote) {
	    console.log("UDP Client: message Rx" + remote.address + ':' + remote.port +' - ' + message + ' | cmd: ' + replyCommandName);
